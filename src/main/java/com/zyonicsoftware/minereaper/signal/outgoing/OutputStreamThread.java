...
public class OutputStreamThread extends RedEugeneSchedulerRunnable {
    ...
    public OutputStreamThread(@NotNull final String eugeneJobName, @NotNull final TimeUnit timeUnit, final long period, final Client client) {
            ...
            if (this.finalOutputStream != null && !this.packets.isEmpty()) {
                ...
                if (packet != null) {
                    ...
                    if (bytes.length < 255) {
                        ...
                        SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).sendPacketMessage(SignalProvider.getSignalProvider().getOutgoingPacketMessage());
                    } else {
                        SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).sendLengthToLargeMessage(SignalProvider.getSignalProvider().getOutgoingLengthToLarge());
                    }
                }
            }
