...
public class ServerSocketAcceptingThread extends RedEugeneSchedulerRunnable {
    ...
    public ServerSocketAcceptingThread(@NotNull final String eugeneJobName, @NotNull final TimeUnit timeUnit, final long period, final ServerSocket serverSocket, final long scheduleDelay) {
    ...
    public void run() {
        ...
        try {
            ...
            if (IPV4AddressInspector.getAcceptedIPAddresses().contains(socket.getInetAddress().getHostAddress())) {
                ...
                SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).acceptSocketConnectionMessage(SignalProvider.getSignalProvider().getAcceptSocketConnectionMessage());
                ...
                SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).unAcceptedSocketConnectionMessage(SignalProvider.getSignalProvider().getUnAcceptSocketConnectionMessage());
                socket.close();
            }
        ...
        for (final Iterator<Client> clientIterator = ServerSocketAcceptingThread.this.clients.listIterator(); clientIterator.hasNext(); ) {
            ...
            if (client != null) {
                if (client.getConnectionUUID().get().equals(uuid)) {
                    try {
                        SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).disconnectClientMessage(SignalProvider.getSignalProvider().getDisconnectClient().replace("%client%", client.getConnectionUUID().get().toString()));
                        client.disconnect();
    ...
    public void disconnectAllClients() {
        ...
        try {
            SignalCallRegistry.getReferenceCaller().getDeclaredConstructor(String.class).newInstance(this.toString()).disconnectAllClientMessage(SignalProvider.getSignalProvider().getDisconnectAllClients());
        } catch (final InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException exception) {
            throw new SignalException(exception);
        }
